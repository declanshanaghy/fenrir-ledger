#!/usr/bin/env node
/**
 * pack-status.js — Full pack status dashboard via GraphQL
 *
 * Usage: node pack-status.js [--status] [--chain-status <N>] [--move <N> <up-next|in-progress|done>]
 *        node pack-status.js [--peek] [--resume-detect <N>] [--epic <label>]
 *
...
function detectEpics(items) {
  const epics = [];

  for (const item of items) {
    if (!item.title.startsWith("Epic:")) continue;
    const body = item.body ?? "";

    // Parse checklist items: - [ ] #N or - [x] #N
    const checklistPattern = /- \[([ x])\] #(\d+)\s*[—–-]?\s*(.*?)(?:\s*\*(.*?)\*)?$/gm;
    const children = [];
    let match;
    ...
    while ((match = checklistPattern.exec(body)) !== null) {
      ...
      const manual = body.includes(`#${num}`) && body.includes("(manual");

      // Detect phase from the item's labels on the board
      const childItem = items.find((i) => i.num === num);
      const phaseLabel = childItem?.labels.find((l) => l.startsWith("phase-"));
      const phase = phaseLabel ? parseInt(phaseLabel.replace("phase-", ""), 10) : null;

      children.push({ num, checked, title: titlePart, manual, phase });
    }

    if (children.length === 0) continue;

    const checkedCount = children.filter((c) => c.checked).length;
    const phases = [...new Set(children.map((c) => c.phase).filter((p) => p !== null))].sort();

    // Current phase = lowest phase with unchecked items
    let currentPhase = null;
    for (const p of phases) {
      if (children.some((c) => c.phase === p && !c.checked)) {
        currentPhase = p;
        break;
      }
    }

    // Blocked items = items in phases > currentPhase (earlier phases not done)
    const blockedItems = [];
    if (currentPhase !== null) {
      for (const child of children) {
        if (child.phase !== null && child.phase > currentPhase) {
          blockedItems.push(child.num);
        }
      }
    }
    ...
    epics.push({
      num: item.num,
      title: item.title.replace("Epic: ", ""),
      children,
      progress: `${checkedCount}/${children.length}`,
      currentPhase,
      blockedItems,
    });
  }
...
function isPhaseGated(issueNum, epics) {
  for (const epic of epics) {
    if (epic.blockedItems.includes(issueNum)) return true;
  }
  return false;
}

function isManualItem(issueNum, items) {
  const item = items.find((i) => i.num === issueNum);
  return (item?.body ?? "").includes("(manual");
}

function getEpicContext(issueNum, epics) {
  for (const epic of epics) {
    const child = epic.children.find((c) => c.num === issueNum);
    if (!child) continue;

    if (epic.blockedItems.includes(issueNum)) {
      const blockers = epic.children
        .filter((c) => c.phase !== null && child.phase !== null && c.phase < child.phase && !c.checked)
        .map((c) => `#${c.num}`);
      return `Phase ${child.phase}, blocked by ${blockers.join(" ")}`;
    }
    if (child.manual) {
      return `Phase ${child.phase ?? "?"} (manual)`;
    }
...
async function statusDashboard() {
  ...
  const result = {
    ...
    up_next_top3: upNext.slice(0, 3).map((i) => ({
      ...
      title: i.title,
      labels: i.labels,
      epic_context: getEpicContext(i.num, epics),
      manual: isManualItem(i.num, items),
      phase_gated: isPhaseGated(i.num, epics),
    })),
    epics: epics.map((e) => ({
      num: e.num,
      title: e.title,
...
async function peek() {
  ...
  const result = upNext.map((item) => ({
    ...
    epic_context: getEpicContext(item.num, epics),
    manual: isManualItem(item.num, items),
    phase_gated: isPhaseGated(item.num, epics),
  }));
...
async function epicView(label) {
  const { items, issueComments, pullRequests } = await fetchBoardAndComments();
  const epics = detectEpics(items);

  // Find epics whose children have the given label, or whose own labels include it
  const matchingEpics = epics.filter((epic) => {
    const epicItem = items.find((i) => i.num === epic.num);
    if (epicItem?.labels.includes(label)) return true;
    return epic.children.some((child) => {
      const childItem = items.find((i) => i.num === child.num);
      return childItem?.labels.includes(label);
    });
  });

  if (matchingEpics.length === 0) {
    console.error(`No epic found with label "${label}"`);
    process.exit(1);
  }

  const result = matchingEpics.map((epic) => {
    // Enrich children with board status and chain info
    const enrichedChildren = epic.children.map((child) => {
      const childItem = items.find((i) => i.num === child.num);
      const boardStatus = childItem?.status ?? "Unknown";
      const comments = issueComments[child.num] ?? [];
      const labels = childItem?.labels ?? [];
      const type = detectType(labels);
      const priority = detectPriority(labels);

      let chainPosition = "Not started";
      if (child.checked || boardStatus === "Done") {
        chainPosition = "Done";
      } else if (boardStatus === "In Progress") {
        const chain = analyzeChain(childItem, comments, pullRequests);
      ...
      return {
        ...
        phase_gated: isPhaseGated(child.num, epics),
      };
    });

    const checkedCount = epic.children.filter((c) => c.checked).length;
    const totalCount = epic.children.length;
    const manualCount = epic.children.filter((c) => c.manual).length;
    const gatedCount = enrichedChildren.filter((c) => c.phase_gated).length;
    const dispatchable = enrichedChildren.filter(
      (c) => !c.checked && !c.manual && !c.phase_gated && c.board_status !== "Done"
    );
    ...
    return {
      ...
      current_phase: epic.currentPhase,
      summary: {
        total: totalCount,
        done: checkedCount,
        manual: manualCount,
        phase_gated: gatedCount,
        dispatchable: dispatchable.length,
      },
      phases: Object.keys(
        epic.children.reduce((acc, c) => {
          if (c.phase !== null) acc[c.phase] = true;
          return acc;
        }, {})
      )
        ...
        .map((phase) => ({
          phase: Number(phase),
          items: enrichedChildren.filter((c) => c.phase === Number(phase)),
        })),
      unphased: enrichedChildren.filter((c) => c.phase === null),
      dispatchable_items: dispatchable.map((c) => ({
        num: c.num,
        title: c.title,
        command: `/fire-next-up #${c.num}`,
      })),
    };
  });
...
async function main() {
  switch (cmd) {
    ...
    case "--epic":
      if (!args[1]) {
        console.error("Usage: node pack-status.js --epic <label>");
        process.exit(1);
      }
      await epicView(args[1]);
      break;
    default:
      console.error(
        "Usage: node pack-status.js [--status] [--chain-status N] [--resume-detect N] [--peek] [--move N <status>] [--epic <label>]"
      );
      process.exit(1);
  }
}
