...
The team has also decided that the Next.js project root will live at `development/src/` so the monorepo can host architecture docs, team SKILLs, and source code alongside each other without collision. Vercel will be configured with Root Directory set to `development/src/`.
...
Run from within `development/src/`. This produces the correct project root structure.
...
- The `development/src/` directory is the Next.js project root — all `npm` commands run from there
- Vercel Root Directory must be set to `development/src/` when hosting is configured
