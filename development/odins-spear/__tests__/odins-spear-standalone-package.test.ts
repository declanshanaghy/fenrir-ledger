 ...
 *   4. Startup sequence — ADC fatal, Firestore non-fatal, Stripe non-fatal
 *   5. Command registration — firestore/stripe command IDs in registry
 *
...
describe("PaletteCommand registry (issue #1496 — commands/registry.ts)", () => {
  ...
  it("accumulates multiple commands in registration order", () => {
    const { registerCommand, getCommands } = makeRegistry();
    registerCommand({ id: "firestore:ping",  label: "Firestore: Ping",  description: "", action: vi.fn() });
    ...
    expect(ids).toEqual(["firestore:ping", "firestore:list", "stripe:check"]);
  });
...
// ─── 4. Startup sequence — connection step ordering ───────────────────────────
//
...
// ─── 5. Command registration — IDs in registry ────────────────────────────────
//
// Validates that the two command registration functions (firestore/stripe)
// register commands with the expected IDs into the shared registry.
// Mirrors commands/firestore.ts, commands/stripe.ts.
...
function registerFirestoreCommands(store: RegistryStore): void {
  store.register({
    id: "firestore:ping",
    label: "Firestore: Ping",
    description: "Ping the Firestore connection (connectivity check)",
    action: vi.fn(),
  });
  store.register({
...
describe("command registration (issue #1496 — commands/firestore, stripe)", () => {
  ...
  it("registerFirestoreCommands registers firestore:ping", () => {
    registerFirestoreCommands(store);
    const ids = store.getAll().map((c) => c.id);
    expect(ids).toContain("firestore:ping");
  });
  ...
  it("both registration functions together populate 3 commands", () => {
    registerFirestoreCommands(store);
    registerStripeCommands(store);
  ...
  it("firestore:ping action is callable and returns a promise or void", async () => {
    const action = vi.fn().mockResolvedValue(undefined);
    store.register({ id: "firestore:ping", label: "Ping", description: "", action });
    const cmd = store.getAll().find((c) => c.id === "firestore:ping");
    await cmd?.action();
    expect(action).toHaveBeenCalledTimes(1);
  });
});
