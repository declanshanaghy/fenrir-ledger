...
// ---------------------------------------------------------------------------
// Screen sub-components (one per step)
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="px-4 py-8 flex justify-center">
      <div className="w-full max-w-[480px] flex flex-col gap-4">
        ...
        <div className="border border-dashed border-destructive p-4 flex gap-3" role="alert">
...
function MergingScreen({ preview }: { preview: HouseholdPreview | null }) {
  return (
    <div className="px-4 py-8 flex justify-center">
      ...
      <div className="w-full max-w-[480px] flex flex-col gap-5">
        <header>
          <h1 className="text-xl font-heading font-bold text-foreground border-b border-border pb-3">
            {hasCards ? "Confirm: Merge Cards \u0026 Join Household" : "Confirm: Join Household"}
          </h1>
        </header>
        ...
        <div className="border border-border p-4 flex flex-col gap-3 items-center">
          ...
          <p className="text-lg font-heading text-muted-foreground" aria-hidden="true">
            ↓ merge into ↓
          </p>
          <div className="border border-border p-3 w-full text-center">
            ...
            <p className="text-xs text-muted-foreground font-body mt-0.5">
              ...
              {MAX_MEMBERS - preview.memberCount} spot remaining · {targetCardLabel}
            </p>
          </div>
        </div>
          ...
          <>
            <div className="border border-border p-3 flex flex-col gap-2">
              ...
              <p className="text-xs text-muted-foreground font-body italic">
                All {preview.userCardCount} card{preview.userCardCount !== 1 ? "s" : ""} from your solo
                household will be transferred.
              </p>
            </div>
            <div className="border border-dashed border-border p-3 flex gap-3" role="alert">
...
/** Primary action button for the code-entry step — state-driven. */
function CodeEntryActionButton({
  validationStatus,
  isComplete,
  isValidating,
  validationMsg,
  ...
  validationMsg: ReturnType<typeof getValidationMessage>;
  onContinue: () => void;
  ...
  if (validationStatus === "already_member") {
    return (
      <button
        type="button"
  ...
  if (validationStatus === "invalid" || validationStatus === "expired" ||
      validationStatus === "full" || validationStatus === "network_error") {
    return (
      <button
        type="button"
...
export default function JoinHouseholdPage() {
  ...
  // Dispatch to screen sub-components based on step
  if (step === "success") {
    return (
      <SuccessScreen
        ...
        preview={preview}
        onDashboard={() => router.push("/ledger")}
      />
    );
  }
  if (step === "race_full") {
    return (
      <RaceFullScreen
        preview={preview}
        onBack={() => router.push("/ledger/settings")}
      />
    );
  }
  if (step === "merge_error") {
  ...
  if (step === "merging") {
    return <MergingScreen preview={preview} />;
  }
