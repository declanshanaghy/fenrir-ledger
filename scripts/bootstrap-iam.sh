...
if [ "$FAILED" -eq 0 ]; then
  ...
  echo "     gh secret set FENRIR_ANTHROPIC_API_KEY    # App runtime — plain API key"
  echo "     gh secret set CLAUDE_CODE_OAUTH_TOKEN     # Agent sandboxes — run: claude setup-token"
  echo "     gh secret set GH_TOKEN_AGENTS             # Agent repo access — GitHub PAT with repo scope"
  echo "     (plus all other secrets listed in deploy.yml header)"
  echo ""
  echo "  2. Run Terraform:"
  echo "     cd infrastructure"
  echo "     terraform init"
  echo "     terraform plan -var billing_account_id=XXXXXX-XXXXXX-XXXXXX"
